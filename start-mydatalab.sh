...
if [ "$(id -u)" -eq 0 ]; then
  NB_UID=${NB_UID:-1000}
  NB_GID=${NB_GID:-100}
  mkdir -p "$POSTGRES_DATA_DIR" "$MINIO_DATA_DIR" "$SUPERVISOR_LOG_DIR" "$SUPERVISOR_RUN_DIR"
  chown -R "$NB_UID:$NB_GID" "$POSTGRES_DATA_DIR" "$MINIO_DATA_DIR" "$SUPERVISOR_LOG_DIR" "$SUPERVISOR_RUN_DIR"
  chmod 700 "$POSTGRES_DATA_DIR" || true
  if [ -d "$WORK_DIR" ] && ! gosu "$NB_UID:$NB_GID" test -w "$WORK_DIR" 2>/dev/null; then
    echo "Work directory $WORK_DIR is not writable for UID $NB_UID." >&2
    echo "Fix permissions on the host (./jupyter-work) before starting the container." >&2
    exit 1
  fi
